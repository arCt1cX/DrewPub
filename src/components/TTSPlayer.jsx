...
export default function TTSPlayer({ getChapterText, onChapterComplete, visible, onClose }) {
    ...
    const handlePlay = useCallback(async () => {
        if (!settings.ttsApiKey) {
            setError('No API key set. Go to Settings → Text-to-Speech to add your Google Cloud TTS key.');
            return;
        }
        ...
        try {
            ...
            const voices = {
                narrator: settings.ttsNarratorVoice || 'en-US-Neural2-D',
                male: settings.ttsMaleVoice || 'en-US-Neural2-J',
                female: settings.ttsFemaleVoice || 'en-US-Neural2-F',
            };

            setLoading(false);
            await engine.start(chunks, settings.ttsApiKey, voices, speed);
        } catch (err) {
